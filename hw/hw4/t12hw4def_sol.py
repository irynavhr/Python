...
res = {"min_of_all":min(list_a), "max_of_all":max(list_a), "max_of_paird":max(list_a[1::2]), 
       "min_of_odd":min(list_a[::2]), "sum_min_paird_and_max_odd":min(list_a[1::2]) + max(list_a[::2]), 
       "max_of_abs":{max(abs_list)}, "max_of_neg":{max(neg_list)}, "exp":{(min_of_all**2) - min(squard_list)}}
